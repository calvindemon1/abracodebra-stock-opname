...
import { Edit, CircleX, Monitor, Plus, Save, X } from "lucide-solid"; // Pake icon Monitor buat PC
  ...
  const submit = async () => {
    if (!name()) {
      return Swal.fire({
        icon: "warning",
        title: "Oops...",
        text: "Nama tipe PC tidak boleh kosong!",
    ...
    try {
      if (editingId()) {
        ...
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
          text: "Tipe PC berhasil diperbarui ✅",
        ...
        Swal.fire({
          icon: "success",
          title: "Berhasil!",
          text: "Tipe PC berhasil ditambahkan ✅",
          confirmButtonColor: "#10b981",
        });
      }
    } catch (err) {
      Swal.fire({
        icon: "error",
        title: "Error!",
        text: "Gagal menyimpan data tipe PC ❌",
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error!",
        text: "Gagal menghapus data tipe PC ❌",
        confirmButtonColor: "#ef4444",
      });
    }
  };
      ...
      >
        ...
        <div class="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">
              Kelola kategori dan jenis perangkat komputer.
            </p>
          </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
          <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">
            {editingId() ? "Edit Type PC" : "Add New Type PC"}
          </label>
          <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
              <input
                ...
                placeholder="e.g. Laptop, Desktop, All-in-One..."
                value={name()}
          ...
          <table class="min-w-full text-sm text-left">
            <thead class="bg-gray-50/80 text-gray-500 text-xs uppercase tracking-widest border-b border-gray-100">
              <tr>
                ...
                <th class="p-4 font-semibold">Type PC Name</th>
            ...
            <tbody class="divide-y divide-gray-50">
              {/* Loading State */}
              <Show when={types.loading && !loading()}>
                <tr>
              ...
              <Show
                when={
                  !types.loading && (!types() || types()?.data?.length === 0)
                }
              >
                <tr>
                  <td colspan="3" class="p-12 text-center text-gray-500">
                    <Monitor size={40} class="mx-auto text-gray-300 mb-3" />
                    <p class="font-medium text-gray-600">
                      Belum ada data tipe PC
                    </p>
                ...
                {(item, i) => (
                  ...
                  >
                    ...
                    <td class="p-4 font-medium text-gray-700 flex items-center gap-3">
                      {/* Aksen titik warna Teal khusus Types PC */}
                      <div class="w-2 h-2 rounded-full bg-teal-300 group-hover:bg-teal-500 transition-colors"></div>
                      {item.type_pc_name}
                    </td>
