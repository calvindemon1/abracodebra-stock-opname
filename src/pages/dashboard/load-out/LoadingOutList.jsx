...
import {
  ...
  Calendar,
  User,
} from "lucide-solid"; // Pake PackageMinus buat Load Out

export default function LoadingOutList() {
  ...
  // FETCH DATA
  const [data, { refetch }] = createResource(() => LoadOutService.list());
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
        text: "Data load-out berhasil dihapus ✅",
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 mt-0.5">
                Daftar pencatatan barang keluar (Load-Out) dari inventory.
              </p>
            </div>
          </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 overflow-hidden">
          ...
          <Show when={!data.loading && (!data() || data().length === 0)}>
            <div class="p-16 text-center text-gray-500">
              <PackageMinus size={48} class="mx-auto text-gray-300 mb-4" />
              <p class="font-medium text-gray-600">
                Belum ada transaksi Loading Out
              </p>
              <p class="text-sm mt-1">
                Klik tombol Create Load-Out untuk mulai mencatat.
              </p>
            </div>
          </Show>
          ...
          <Show when={!data.loading && data()?.length > 0}>
              ...
              <table class="min-w-full text-sm text-left whitespace-nowrap">
                <thead class="bg-gray-50/80 text-gray-500 text-xs uppercase tracking-widest border-b border-gray-100">
                  <tr>
                    ...
                    <th class="p-4 font-semibold">PIC Load-Out</th>
                      ...
                      >
                        ...
                        <td class="p-4 text-center">
                          <div class="flex items-center justify-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              onClick={() =>
                                navigate(`/admin/load-out/${row.id}`)
                              }
            ...
            <div class="p-4 border-t border-gray-100 bg-gray-50/30 text-xs text-gray-400 font-medium text-right">
              Total: {data()?.length || 0} Data Load-Out
            </div>
          </Show>
        </div>
      </div>
